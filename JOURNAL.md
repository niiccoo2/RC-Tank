...
- [x] Stop motors after self driving ends
- [ ] Better debug system
- [ ] RTK GPS
...
| Failsafe               | Seemed like it should have caught the past issues                   | [x]   |
...
Fixing the timeout was super easy, and now that I think about it, it also should be a janky fix to the ESC's turning off. I just removed the thing that makes sure it isn't already stopped. I don't even know why I had that, because it doesn't really do anything with how the program is set up right now.
...
Done for now. It isn't working, I think we are making progress though. Not sure if the GPS isn't accepting the corrections, or if we aren't getting them from the server. Action points for next time will be:
...
Apparently, they never gave me the free RTK connection, so I now have to go back to MACors for now and email them.
