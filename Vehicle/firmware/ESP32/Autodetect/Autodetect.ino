...
// ------- BOARD-SPECIFIC SERIAL SETUP --------
#if defined(ARDUINO_ARCH_ESP8266)

// ESP8266: use SoftwareSerial for hoverboard
#include <SoftwareSerial.h>
const int pinRX = D5, pinTX = D6;   // pick whatever pins you actually wired
SoftwareSerial oSerialHover(pinRX, pinTX); // RX, TX

#elif defined(ESP32)

// Original ESP32 block (unchanged)
const int pinRX = 12, pinTX = 14;
#define oSerialHover Serial1    // ESP32

#else

// Original "Arduino" block (unchanged)
#include <SoftwareSerial.h>    
...
void setup() 
{
  ...
  // ------- BOARD-SPECIFIC begin() CALL -------
  #if defined(ARDUINO_ARCH_ESP8266)

    // ESP8266 SoftwareSerial: only needs baud rate
    oSerialHover.begin(BAUDRATE);

  #elif defined(ESP32)

    // Original ESP32 call (unchanged)
    oSerialHover.begin(BAUDRATE, SERIAL_8N1, pinRX, pinTX);

  #else

    // Original "Arduino" call (unchanged)
